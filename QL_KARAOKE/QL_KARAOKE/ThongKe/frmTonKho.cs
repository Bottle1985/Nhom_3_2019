...
namespace QL_KARAOKE.ThongKe
{
    public partial class frmTonKho : Form
    {
        ...
        private void frmTonKho_Load(object sender, EventArgs e)
        {
            ...
            //ẩn cột không cần thiết hiển thị
        ...
        private void ThongKe(int dieuKien)
        {
            ...
            if(dieuKien == 0)//lấy tất cả các mặt hàng khi số lượng đã hết
            {
                var result = tonkhoHang.Where(x=>x.tonkho == 0);
                dgvTonKho.DataSource = result;
                return;//không cần quan tâm các câu lệnh phía dưới
            }
            if(dieuKien == 1)
            {
                dgvTonKho.DataSource = tonkhoHang;
                return;//không quan tâm các câu lệnh tiếp theo
            }
